...
from DES_Python.rngs import random
from math import log, sqrt, exp

from DES_Python.rvms import cdfNormal, idfNormal
...
def Normal(m, s):
    ...
    """
    La funzione genera un numero casuale u utilizzando il generatore 
    di numeri pseudo-casuali. Questo numero determina se la distribuzione 
    è simmetrica (caso u < 0.5) o asimmetrica (caso u >= 0.5).
    """
    u = random()
    if (u < 0.5):
        t = sqrt(-2.0 * log(u))
    ...
    """
    Infine, il tempo di servizio effettivo è calcolato come la somma di m 
    (media) e il prodotto tra s (deviazione standard) e 
    z (deviazione standard standardizzata).
    """
    return (m + s * z)
